...
const SAMPLE = {
  ...
  matches_counted: 3,
  metrics: [
    { metric: "win_share", value: 66.67, opposed_value: null },
    { metric: "goals", value: 2.33, opposed_value: 0.67 },
  ],
};
...
const FORM_PAYLOAD = {
  ...
  home: { team_id: 3, samples: [SAMPLE] },
  away: { team_id: 4, samples: [SAMPLE] },
  families: [{ family: "result", metrics: ["win_share", "goals"] }],
};

describe("getFixtureForm", () => {
  afterEach(() => {
  ...
  it("reports nothing without requesting when the backend URL is blank", async () => {
    ...
    const result = await getFixtureForm(FIXTURE_ID);
  ...
  /**
   * GIVEN an API answering the form request with a server error
   * WHEN a fixture's form is read
  ...
  it("normalizes a non-OK status into the unavailable result", async () => {
    ...
    await expect(getFixtureForm(FIXTURE_ID)).resolves.toEqual({
      loaded: false,
  ...
  /**
   * GIVEN an API answering the form request with a body that is not JSON
   * WHEN a fixture's form is read
   ...
   * WHEN a fixture's form is read
   * THEN it is refused rather than read as a fixture with nothing played
   */
  ...
  /**
   * GIVEN a reachable API holding form for the fixture
   * WHEN a fixture's form is read
   * THEN the fixture's address is requested with the token and a deadline
   */
  it("reads the fixture's form with the session token", async () => {
    const fetchStub = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
      json: async () => FORM_PAYLOAD,
    });
    ...
    expect(fetchStub).toHaveBeenCalledWith(
      "http://api.test/api/v1/fixtures/41/form",
