...
**Before this is deployed**, three requirements. The edge in front of the frontend must set `X-Forwarded-For` from the connecting address: Next.js fills the header in from the socket only when the request carries none, so a directly exposed frontend would pass a visitor-chosen value straight through and let one client mint unlimited buckets. The peer addresses must be listed, or every visitor is attributed to the frontend tier and shares one bucket — which is why `preproduction` and `production` refuse to boot without `DJANGO_TRUSTED_PROXY_NETWORKS`, the same way they refuse to boot without a secret key. And `DJANGO_TRUSTED_PROXY_HOPS` must match the topology: count every hop between the visitor and the API that appends to the chain, and remember the frontend itself appends nothing, since it forwards what it received. Setting it too low attributes visitors to a proxy, which is visible in the logs as the same address on every line; setting it too high attributes them to an entry they supplied, which is a throttle in name only.
