...
The result is not stored. Goals, and therefore the win, draw and loss shares, come from `Fixture.home_goals` and `Fixture.away_goals`, which are already synchronized and already guarded as a pair. The provider withholds its own goals statistic for some matches, so a table sourcing goals from the statistics would read those as missing data.

What a withheld statistic means took two corrections to get right, and the answer decides how much history is usable. Measured over the 7,100 sides of the 3,550 matches between July 2024 and August 2026, a statistic is always withheld for the whole match and never for one side of it: not one type was absent from a single side while present on the other. So an absence says nothing about a particular club, and the only question is whether nought is a reading it could stand for.

For ten types the question never arises, because they are never absent. For eight it is answered yes, because the provider also sends explicit noughts for them — offsides reads nought on 1,247 sides, red cards on 558, blocked shots on 466 — so an absence is read as nought. That is a deliberate trade rather than a contract, and it is the right way round: refusing the record instead would discard twenty-one sound figures over one, and would cost 83 percent of matches for red cards alone. For four it is answered no. Duels won never once reads nought across 6,106 sides, its lowest reading is three and its median forty-seven, so the 497 matches it is missing from cannot be matches where both clubs won none; tackles and dribbles attempted have lowest readings of one and no nought either. Those four columns are nullable, and a metric whose column is null in every match a sample counted is left out of that sample rather than published as nought.

Getting that wrong is expensive rather than academic. Treating duels won as required discarded 499 of 3,548 finished fixtures, fourteen percent, over one absent figure in an otherwise complete record.
...
```bash
docker compose --env-file .env.local exec worker uv run celery -A config call statistics.synchronize_statistics --args='[772]'
```

Seven hundred and seventy-two days reaches the start of the 2024/2025 season, which is as far back as the subscription serves: reads were verified to return complete windows from August 2024 and to be refused for every month before it. Three seasons of depth is not there so that a window can span them: it is there so that opening a fixture played in an earlier season reads that season's earlier matches, which is what makes a backward-looking study possible at all. That is roughly 3,500 finished fixtures over twenty-six chunks and around seventy seconds, against a budget of 2000 calls per entity per hour. Re-running it is free: the upsert is keyed on the fixture and the team, and reconciliation is by stamp identity over exactly the fixtures the run resolved, so a match the provider stops publishing statistics for loses its rows while nothing else is touched.
