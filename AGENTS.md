...
- Classes document non-method attributes in an `Attributes` section, enum members included with their serialized value; and every public method it defines in a `Methods` section as a full signature including the return type, which is stricter than numpydoc deliberately, since a growth threshold is not evaluable and a truncated signature is arbitrary; private methods are excluded.
